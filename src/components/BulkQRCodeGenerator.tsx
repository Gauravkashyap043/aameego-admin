...
const BulkQRCodeGenerator: React.FC<BulkQRCodeGeneratorProps> = ({
  ...
  const handleSelectAllFiltered = () => {
    ...
    const selectedFiltered = filteredVehicles.filter((v) =>
      selectedVehicles.has(v._id)
    );
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text(
        `Total Vehicles: ${selectedVehiclesList.length}`,
        pageWidth / 2,
        32,
        { align: "center" }
      );
      ...
      // Save the PDF
      const fileName = `vehicle-qr-codes-${
      ...
      <div className="bg-white rounded-xl p-8 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-gray-700 mb-2">
                Select vehicles to generate QR codes for. Currently{" "}
                <strong>{selectedVehicles.size}</strong> of{" "}
                <strong>{vehicles.length}</strong> vehicles selected.
              </p>
              <p className="text-sm text-gray-500">
                Each QR code will contain the vehicle number and can be scanned
          ...
          {progress > 0 && isGenerating && (
            <p className="text-sm text-gray-600 text-center">
              ...
              {Math.round((progress / 100) * selectedVehicles.size)} of{" "}
              {selectedVehicles.size} vehicles)
            </p>
          )}
        </div>
            ...
            filteredVehicles.map((vehicle) => (
              ...
              >
                ...
                <div className="bg-white p-2 rounded border">
                  <QRCode value={vehicle.vehicleNumber} size={60} level="M" />
                </div>
                <div className="mt-2">
        ...
        <div className="mt-6 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">
            ...
            highlighted with a blue border. Unselected vehicles appear dimmed.
          </p>
        </div>
      </div>
    </div>
